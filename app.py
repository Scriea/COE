...
def get_response(user_query):
    ...
    hallucination_probabilities = hallucination_checker.hallucination_prop(
        response, context="joint_passages"
    )
...
## Processing Document
passages = attribution_module.load_paragraphs(passages_file=PASSAGE_FILE)
if passages:
    joint_passages = "\n".join(passages)
    if not os.path.exists(EMBEDDINGS_FILE):
        attribution_module.vectorize_paragraphs(PASSAGE_FILE, EMBEDDINGS_FILE)

search_index, paragraphs = attribution_module.create_faiss_index(
    embedding_file_path=EMBEDDINGS_FILE, ngpu=1
)
