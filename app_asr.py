...
if "user_lang" not in st.session_state:
    st.session_state.user_lang = 1  # "English" is the default language
...
def clear_chat_history():
    # Reset only chat-related session states
    st.session_state.chat_history = [{"role": "assistant", "content": "How may I assist you today?"}]
    st.session_state.passages = None
    st.session_state.search_index = None
    st.session_state.paragraphs = None
    st.session_state.pdf_processed = False
    # Keep audio states as they are to prevent reprocessing
...
with st.sidebar:
    ...
    st.session_state.user_lang = lang_index[st.selectbox("Select Language for Transcription", ["English", "Hindi", "Tamil"])]

    # Check if the language has changed
    if prev_lang != st.session_state.user_lang:
        st.session_state.display_audio = False  # Do not display audio and transcribed text if the language changes
    ...
    if len(audio) > 0:
        ...
        if st.session_state.audio_timestamp is None or st.session_state.audio_timestamp != current_timestamp:
            ...
            st.session_state.display_audio = True  # Set flag to control display of audio and text
...
# Display or clear chat messages first to avoid order issues
...
# Automatically enter transcribed text into chat input if available
